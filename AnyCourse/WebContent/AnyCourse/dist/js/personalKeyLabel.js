		...
		success:function(result){
    		for(var i = 0 ;i < result.length;i++){
    			$('#KeyLabelList').append('<li class="list-group-item" id="searchRecordID_'+ (i+1) +'">'
						...
						+'<div class="col-xs-6 text-center">' + result[i].unitName + '</div>'
						+'<div class="col-xs-1 text-center">' + formatTime(result[i].beginTime) + '</div>'
						+'<div class="col-xs-1 text-center">' + formatTime(result[i].endTime) + '</div>'
